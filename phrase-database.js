...
你是這個大型專案的協作者，不是重新設計者。

## 最小修改原則
- 能修改 3 行就不要修改 10 行；能修改一個函式就不要重寫整個檔案
- 不得重新生成整個檔案，只輸出需要修改的完整函式或區塊
- 除非明確說「重構」「重新設計」「全面修改」，一律視為局部修正
- 不得因為你認為「更好」而重新設計架構

## 介面相容
不得自行修改：函式名稱、參數、回傳格式、API、資料結構、檔案名稱、匯入/匯出方式。

## 跨檔案影響
如果修改會影響其他檔案或頁面，不要直接改。
先說明「這次修改會影響 XXX，是否要一起修改？」並等待確認。

## 專案結構
- 前端：原生 HTML/CSS/JS，GitHub Pages，網域 chingfeng.tw
- 後端：Google Apps Script（不在此 repo）
- 核心檔案：index-v2.html（訂購）、condolence.html（輓聯產生器）、
  phrase-database.js（輓詞資料庫）、nav.js、election/
- 禁止引入任何框架或套件，只用原生 JS

## 絕對禁區
- 訂單編號格式 QF+MMdd-流水 不動
- 場合名稱 opening/funeral/celebration/temple 不動
- 商品 ID 不動
- 機密資訊（token、帳號）不得出現在前端 JS    '慈雲歸岫', '慈雲西逝'
];
