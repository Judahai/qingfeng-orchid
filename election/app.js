...
async function fetchCandidates(force = false) {
  ...
  if (force && grid) grid.innerHTML = '<div class="loading-state">候選人資料重新載入中…</div>';
  try {
    const data = await window.SITE_DATA.fetchJson(CONFIG.GAS_URL + '?action=candidates', {
      cacheKey: 'candidates',
      force,
    });
    if (!Array.isArray(data)) throw new Error('候選人資料格式不正確');
