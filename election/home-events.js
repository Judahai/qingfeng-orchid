...
async function fetchHomeEvents(force = false) {
  const track = document.getElementById('homeEventTrack');
  if (!track) return;
  if (force) track.innerHTML = '<div class="loading-state">活動重新載入中…</div>';

  try {
    const data = await window.SITE_DATA.fetchJson(HOME_EVENTS_CONFIG.GAS_URL + '?action=events', {
      cacheKey: 'events',
      force,
    });
    ...
    track.innerHTML = `
      <div class="home-event-card home-event-more">
        <strong>活動暫時無法載入</strong>
        <button type="button" class="btn-retry" id="retryHomeEvents">重新載入</button>
        <a href="events.html">前往活動頁 →</a>
      </div>`;
    document.getElementById('retryHomeEvents')?.addEventListener('click', () => fetchHomeEvents(true));
  }
}
